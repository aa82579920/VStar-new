...
class AVPlayerView: UIView {
    ...
    func initSubView() {
        ...
        cancelLoadingQueue = DispatchQueue.init(label: "com.duanqu.demo.play")
    }
    ...
    func setPlayerSourceUrl(url:String) {
        ...
        let components = URLComponents.init(url: sourceURL!, resolvingAgainstBaseURL: false)
        ...
        queryCacheOperation = WebCacheManager.shared().queryURLFromDiskMemory(key: cacheFileKey ?? "", cacheQueryCompletedBlock: { [weak self] (data, hasCache) in
            DispatchQueue.main.async {[weak self] in
                if !hasCache {
                    self?.sourceURL = self?.sourceURL?.absoluteString.urlScheme(scheme: "http")
                } else {
                ...
                if let url = self?.sourceURL {
                    ...
                    self?.urlAsset?.resourceLoader.setDelegate(self, queue: .main)
