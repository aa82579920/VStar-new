...
class TNDatePicker: UIViewController {
    ...
    var containV:UIView = {
        let view = UIView(frame: CGRect(x: 0, y: Screen.height - 240, width: Screen.width, height: 240))
    ...
    private func drawMyView() {
        ...
        let sure = UIButton(frame: CGRect(x: Screen.width - 80, y: 10, width: 70, height: 20))
        sure.setTitle("确认", for: .normal)
        sure.setTitleColor(.starRed, for: .normal)
        sure.addTarget(self, action: #selector(self.onClickSure), for: .touchUpInside)
        picker = UIPickerView(frame: CGRect(x: 0, y: 24, width: Screen.width, height: 216))
...
extension TNDatePicker: UIPickerViewDelegate,UIPickerViewDataSource {
...
// MARK: - 转场动画delegate
extension TNDatePicker: UIViewControllerTransitioningDelegate {
    func animationController(forPresented presented: UIViewController, presenting: UIViewController, source: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        let animated = TNDatePickerAnimated(type: .present)
        return animated
    }
    func animationController(forDismissed dismissed: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        let animated = TNDatePickerAnimated(type: .dismiss)
        return animated
    }
}
